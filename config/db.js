  ...
  if (!mongooseConnectionPromise) {
    ...
    conn.on("error", (err) => {
      ...
      if (rejectNativeDbPromise) {
        // Check if reject function exists
        // Avoid rejecting if promise already settled
        nativeDbReadyPromise
          .catch(() => {})
          .finally(() => rejectNativeDbPromise(err));
      }
    });
    ...
    conn.on("reconnected", () => {
      ...
      try {
        ...
        if (conn.client && typeof conn.client.db === "function" && dbName) {
          ...
          const MongodbNativeDbClass =
            conn.client.constructor.Db ||
            (tempDb && Object.getPrototypeOf(tempDb).constructor);

          if (
            tempDb &&
            MongodbNativeDbClass &&
            tempDb instanceof MongodbNativeDbClass
          ) {
            nativeDbForGridFS = tempDb;
            console.log(
              "[DATABASE] Native Db instance for GridFS RE-CAPTURED after reconnect."
            );
            ...
            console.error(
              "[DATABASE] Recaptured conn.client.db() is NOT a valid Db instance after reconnect."
            );
          }
        } else {
          console.error(
            "[DATABASE] Cannot recapture native Db instance after reconnect (client or dbName missing)."
          );
        }
      } catch (e) {
        console.error(
          "[DATABASE] Error recapturing native Db after reconnect:",
          e
        );
      }
    });
    ...
    conn.once("open", () => {
      console.log(
        `[DATABASE] Mongoose connection 'open' event fired. DB Name from conn: ${conn.name}`
      );
      try {
        const dbName = conn.name;
        if (!dbName) {
          const errMsg =
            '[DATABASE] "open" event fired, but conn.name (database name) is missing.';
          console.error(errMsg);
        ...
        if (conn.client && typeof conn.client.db === "function") {
          ...
          const MongodbNativeDbClass =
            conn.client.constructor.Db || // Ideal path
            (tempDb && Object.getPrototypeOf(tempDb)
              ? Object.getPrototypeOf(tempDb).constructor
              : null); // Fallback
          ...
          if (
            ...
            nativeDbForGridFS = tempDb;
            console.log(
              "[DATABASE] Native Db instance for GridFS CAPTURED successfully via conn.client.db()."
            );
            ...
            console.error(
              errMsg,
              "tempDb constructor:",
              tempDb ? tempDb.constructor.name : "N/A",
              "Expected Db constructor:",
              MongodbNativeDbClass ? MongodbNativeDbClass.name : "N/A"
            );
            console.error(
              "Logged tempDb structure (first 1000 chars):",
              JSON.stringify(tempDb, null, 2).substring(0, 1000) + "..."
            );
            if (rejectNativeDbPromise) {
          ...
          const errMsg =
            '[DATABASE] "open" event, but conn.client or conn.client.db is not available/valid.';
          console.error(
            errMsg,
            "conn.client exists:",
            !!conn.client,
            "typeof conn.client.db:",
            typeof conn.client?.db
          );
          if (rejectNativeDbPromise) {
            rejectNativeDbPromise(new Error(errMsg));
        ...
        const errMsg =
          '[DATABASE] Exception during "open" event native Db capture: ' +
          e.message;
    ...
    mongooseConnectionPromise = mongoose
      ...
      .then((mongooseInstance) => {
        console.log(
          `[DATABASE] mongoose.connect() promise resolved. Host: ${mongooseInstance.connection.host}, DB Name: ${mongooseInstance.connection.name}`
        );
      ...
      .catch((error) => {
        console.error(
          `[DATABASE FATAL] Could not connect to MongoDB (mongoose.connect catch): ${error.message}`
        );
...
const createGridFsStorage = () => {
  if (!process.env.MONGO_URI) {
    const errMsg =
      "[GridFS Storage Config] MONGO_URI not found. GridFS storage cannot be initialized.";
    console.error(errMsg);
  ...
  connectDB().catch((err) => {
    console.error(
      "[GridFS Storage Config] Initial connectDB() call failed:",
      err.message
    );
  ...
  console.log(
    "[GridFS Storage Config] GridFsStorage configured to wait for nativeDbReadyPromise."
  );
