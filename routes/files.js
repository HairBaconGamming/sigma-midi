...
router.get('/stream/:fileIdOrFilename', async (req, res) => {
  try {
    ...
    console.log(`[Files Route /stream] Attempting to stream file with identifier: ${identifier}`);
    ...
    const files = await fileCursor.toArray();
    if (!files || files.length === 0) {
      console.log(`[Files Route /stream] No file found for identifier: ${identifier}`);
      return res.status(404).json({ msg: 'No file exists with that identifier.' });
    }
    const file = files[0];
    console.log(`[Files Route /stream] File found: ${file.filename}, contentType: ${file.contentType}, length: ${file.length}`);
    ...
    readStream.on('error', (streamErr) => {
      console.error(`[Files Route /stream] GridFS stream error for file ${file.filename} (ID: ${file._id}):`, streamErr);
      if (!res.headersSent) {
        res.status(500).send('Error streaming file.');
      } else {
        readStream.destroy();
    ...
    console.error('[Files Route /stream] General error:', err.message, err.stack);
    if (!res.headersSent) {
        if (err.name === 'MongoNetworkError' || err.name === 'MongooseServerSelectionError') {
            return res.status(503).send('Database connection error.');
        }
